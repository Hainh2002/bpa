...
/* Actual period: 0.001 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0xea60,
  .duty_cycle_counts = 0x7530, .source_div = (timer_source_div_t) 0, .channel = 0, .p_callback = timer3_ISR,
...
{ .data_flash_bgo = true, .p_callback = ISR_Flash, .p_context = NULL,
...
{ .low_power_mode = LPM_MODE_SLEEP,
  ...
  .standby_wake_sources = LPM_STANDBY_WAKE_SOURCE_IRQ7 | (lpm_standby_wake_source_t) 0,
...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.001 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x3a98,
  .duty_cycle_counts = 0x1d4c, .source_div = (timer_source_div_t) 0, .channel = 2, .p_callback = Timer_ISR2,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.001 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x3a98,
  .duty_cycle_counts = 0x1d4c, .source_div = (timer_source_div_t) 0, .channel = 1, .p_callback = Timer_ISR1,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
{ .unit = 0, .mode = ADC_MODE_SINGLE_SCAN, .resolution = ADC_RESOLUTION_12_BIT, .alignment =
  ...
  .trigger = ADC_TRIGGER_SOFTWARE, .p_callback = ADC_ISR, .p_context = NULL, .p_extend = &g_adc0_cfg_extend,
...
{ .scan_mask = ADC_MASK_CHANNEL_11 | ADC_MASK_CHANNEL_16 | ADC_MASK_TEMPERATURE | ADC_MASK_VOLT | 0,
  .scan_mask_group_b = 0,
...
/* Baud rate calculated with 0.160% error. */.abcse = 0,
  .abcs = 0, .bgdm = 0, .cks = 0, .brr = 194, .mddr = (uint8_t) 256, .brme = false };
...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.01 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x493e,
  .duty_cycle_counts = 0x249f, .source_div = (timer_source_div_t) 3, .channel = 0, .p_callback = Timer_ISR,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
can_bit_timing_cfg_t g_can0_bit_timing_cfg =
        {
          /* Actual bitrate: 500000 Hz. Actual Bit Time Ratio: 73 %. */.baud_rate_prescaler = 1 + 1 /* Division value of baud rate prescaler */,
          .time_segment_1 = 10,
          .time_segment_2 = 4,
